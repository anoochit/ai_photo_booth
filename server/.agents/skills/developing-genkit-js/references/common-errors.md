...
- **Preferred**: Use latest model aliases (e.g. `gemini-flash-latest`) for best performance and features.
  ...
  ```
- **DEPRECATED**: Versioned legacy models (like `gemini-3.5-flash` or `gemini-2.5-flash`) should be updated to latest aliases.
  ```ts
  model: googleAI.model('gemini-3.5-flash') // DEPRECATED (Use gemini-flash-latest instead)
  ```
